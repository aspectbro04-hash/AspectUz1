...
export const GameProvider = ({ children }: { children: ReactNode }) => {
  ...
  const createRoom = (playerName: string) => {
    if (!socket) return;
    socket.emit('create_room', { playerName, playerId }, (response: { roomId: string }) => {
  ...
  const joinRoom = (roomId: string, playerName: string) => {
    if (!socket) return;
    socket.emit('join_room', { roomId, playerName, playerId }, (response: { success: boolean, message?: string }) => {
      if (!response.success) {
